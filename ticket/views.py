...
GeneralChangeStatus_uri = os.path.join(settings.BASE_DIR, "ticket/import", "General Support Status change as Closed_Cases__1.csv") 

# Create your views here.
def test(req):
    print(req.user.id)
    # ticeket_resp = functions.extract_csv(GeneralChangeStatus_uri, 0, 'ChangeStatus', 1)
    return JsonResponse({"message": "ticeket_resp"})
...
def changestatus(request):
    ...
    if oauth_obj.updated_time < expiry_time:
        ...
        print("token already updated")
        print("Processing state----")
        schedule_obj, created = models.CsvSchedule.objects.get_or_create(oauthModal=oauth_obj, id=1,
        defaults={
            "name": "faiyas",
            "last_row": 0,
            "last_run": timezone.now(),
        ...
        if schedule_obj.last_run  < timezone.now():
            ticeket_cnt = functions.extract_csv(GeneralChangeStatus_uri, schedule_obj.last_row, 'ChangeStatus', user_id)
            schedule_obj.last_run = timezone.now() + timedelta(seconds=25)
            schedule_obj.last_row = ticeket_cnt
            schedule_obj.save()
            print("Completed")
        else:
            print("get")
